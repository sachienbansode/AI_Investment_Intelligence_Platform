...
def logo_url() -> str:
    base = app_url()
    return (base + "/niytri-logo.png") if base else ""
...
def _layout(inner_html: str) -> str:
    import datetime
    plat, url, logo, year = platform_name(), app_url(), logo_url(), datetime.datetime.now().year
    header = (f'<img src="{logo}" alt="{_html.escape(plat)}" height="38" style="display:block;border:0;height:38px">'
              if logo else
              f'<div style="font-size:22px;font-weight:800;color:#F94C00;font-family:Arial,Helvetica,sans-serif">{_html.escape(plat)}</div>')
    return (
