...
function Consent({ brand, onAccept, onSignOut }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 20, zIndex: 100, overflow: 'auto' }}>
      <div style={{ maxWidth: 560, background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 14, padding: 24 }}>
        <h2 style={{ margin: '0 0 6px', fontSize: 20 }}>Accept our Terms & Conditions</h2>
        <p style={{ color: 'var(--muted)', margin: '0 0 14px', fontSize: 14 }}>Please review and accept to continue using {brand}.</p>
        <div style={{ background: 'var(--panel2)', border: '1px solid var(--border)', borderRadius: 10, padding: 14, fontSize: 13.5, lineHeight: 1.6, color: 'var(--text)' }}>
          All AI outputs in this application — scores, insights, summaries and chat responses — are generated by
          artificial intelligence for informational purposes only. They are not investment advice, research reports, or
          recommendations to buy or sell securities, and must be reviewed and approved before business or regulatory use.
          Investments in securities markets are subject to market risks. Please consult a SEBI-registered investment
          adviser before investing.
        </div>
        <p className="hint" style={{ margin: '12px 0 16px' }}>By clicking Accept you agree to our Terms & Conditions and confirm you understand the above.</p>
        <div style={{ display: 'flex', gap: 10 }}>
          <button disabled={busy} onClick={async () => { setBusy(true); try { await onAccept() } finally { setBusy(false) } }}>{busy ? 'Please wait…' : 'Accept & continue'}</button>
